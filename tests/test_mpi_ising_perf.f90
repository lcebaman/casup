!*robodoc*u* tests/test_mpi_ising_perf
!  NAME
!    test_mpi_ising_perf
!  SYNOPSIS

!$Id: test_mpi_ising_perf.f90 536 2018-04-03 12:02:13Z mexas $

program test_mpi_ising_perf
...
do run=1,3
  select case(run)
  case(1)
    call ca_mpi_ising_energy( space=space, iter_sub = ca_iter_tl,       &
      kernel = ca_kernel_ising_ener, energy=energy0, magnet=magnet0 )
  case(2)
    call ca_mpi_ising_energy( space=space, iter_sub = ca_iter_dc,       &
      kernel = ca_kernel_ising_ener, energy=energy1, magnet=magnet1 )
  case(3)
    call ca_mpi_ising_energy( space=space, iter_sub = ca_iter_omp,      &
...
end program test_mpi_ising_perf
